# EE232E Homework 1 Problem 7 Part 2
...
for (i in 1:nrow(file)) {
    for(j in 1:nrow(file)) {
        ...
        if (result <= 0.3) {
            matrix_coefficient[i, j] <- result
        } else {
            matrix_coefficient[i, j] <- -1
        }
    }
}
...
write.graph(graph, file = "/home/weikun/Downloads/finance_data/graph3.txt", format = "ncol")
